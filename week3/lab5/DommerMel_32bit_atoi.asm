...
######################################################################
		.data
buffer:		.space 		11
instructions:	.asciiz		"You can enter any positive number from 0 to 4294967295 and this program will convert the string to integer.\n"
prompt:		.asciiz		"Enter your number (0-4294967295): "
error_char:	.asciiz		"\nYou didn't enter a valid number\nUse only 0-9. Try again.\n"
error_ovflow:	.asciiz		"\nYou entered a number that is too large to fit into 32-bits (0-4294967295). Try again.\n"
...
print_results:	li	$v0,	4			# print the finished text
		la	$a0,	finished
		syscall
		li	$v0,	36			# print the unsigned integer
		move	$a0,	$t1
		syscall
...
##################################################################
# ATOI (32-bit version)                                          #
# Function to convert a string to unsigned 32-bit integer.       #
#                                                                #
# Arguments:                                                     #
...
convert:	move	$t3,	$zero			# initialize $t3 to 0
		li	$t4,	10			# set $t4 to 10 for muliplication base 10 adjustment
		multu	$t1,	$t4			# multiply current result by 10
		mfhi	$t4				# load HI into t4 to check for overflow.
		bnez	$t4,	overflow		# return overflow error if needed.
		mflo	$t1				# move LO into $t1 (result)
		addu	$t3,	$t1,	$t2		# add the current digit to the result into t3 to check for overflow against $t1
		sltu	$t4,	$t3,	$t1		# check if overflow occurred.
		bnez	$t4,	overflow		# return overflow error if needed
